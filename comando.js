export default async function handler(req, res) {
  // Configura CORS para permitir chamadas do seu site
  ...
  Sua função é interpretar a intenção e responder EXCLUSIVAMENTE em formato JSON (sem marcação Markdown ou bloco de código).
  ...
  try {
    ...
    const rawAnswer = data.candidates[0].content.parts[0].text.trim().replace(/^```json/, '').replace(/```$/, '').trim();
